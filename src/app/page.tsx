    ...
    <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-24 px-6 py-24">
      ...
      <section className="flex flex-col gap-6">
        <Badge variant="primary" className="w-fit">
          23 components · 1 token file
        </Badge>
        <h1 className="max-w-2xl font-display text-display-xl text-ink-950">
          A component system drafted like a blueprint, not assembled from
          defaults.
        </h1>
        <p className="max-w-xl text-body-lg text-ink-600">
          Vellum is a token-driven React library — every color, radius, and
          spacing value traces back to a single source, so the interface stays
          consistent by construction. Built on Radix UI, Tailwind v4, and Framer
          Motion, documented in Storybook.
        </p>
      ...
      <section className="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Color, type, spacing, radius, shadow, and motion — one file, every
              component reads it.
            </CardDescription>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              From Button to DatePicker, each built on Radix UI for correct
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Every component ships with Storybook stories covering its real
              states and variants.
            </CardDescription>
          </CardHeader>
        </Card>
      ...
      <section id="catalog" className="flex flex-col gap-6">
        <h2 className="font-display text-heading-lg text-ink-950">
          The catalog
        </h2>
        <div className="flex flex-wrap gap-2">
          {components.map((name) => {
            ...
            return (
              <a
                key={name}
                href={`${STORYBOOK_URL}/?path=/docs/components-${slug}--docs`}
                target="_blank"
                rel="noreferrer"
              >
                <div
                  className="rounded-md border px-3 py-2 transition-colors
        duration-200 hover:border-(--color-primary-600)
        hover:bg-(--color-primary-600) hover:text-white"
                >
                  {name}
                </div>
              </a>
      ...
      <footer className="border-t border-ink-100 pt-8 text-body-sm text-ink-400">
        Run{" "}
        <code className="font-mono text-mono-sm text-ink-950">
          npm run storybook
        </code>{" "}
        to browse every component, variant, and token live.
      </footer>
    </main>
  );
}
